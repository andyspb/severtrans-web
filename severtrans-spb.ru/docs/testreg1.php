...
$message="Здравствуйте. Вам выставлен СЧЕТ, за грузоперевозку <br/>";
	...
	while($row5 = mysql_fetch_assoc($query5))
        {$sender5 = $row5['sender5'];
		  $message=$message."  "."Номер отправки: ".$row5['number']."  "." в город  ".$row5['destination'].", "." отправитель  "."  ".$row5['sender'].",   "."получатель   "."  ".$row5['receiver']." <br/>  "." вес - ".$row5['weight']." кг.;  объем - ".$row5['volume']." м. куб.,  кол-во мест -".$row5['seats'].", стоимость перевозки:  ".$row5['total']." руб "."<br/>";}
		  };
...
$out2 = num2str($sum, true);
		...
		$kk=$kk.$ii++."  "."Счет для  ".$alias."  "."№".$num."  "."на адрес ".$to." результат - ".$error."<br />";
							}
else
{
	  $message =$message."<br>ВНИМАНИЕ!!!<br> Изменились реквизиты банка. Вместо ЗАО теперь ПАО"."<table width='800' border='0'>
	<tr height='40'>
    <td colspan='2' style='font-family: Impact; font-size:16px;'><div align='center'>Счет № $num от $data</div></td>
  </tr>
  <tr height='15'>
...
$headers = "MIME-Version: 1.0\r\n";
 		  $headers .= "Content-type: text/html; charset=utf-8\r\n";
 		   $send = mail ($from, $sub1, $kk, $headers);
	}
?>
</body>
</html>